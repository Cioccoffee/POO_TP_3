...
static void read(ListeTrajets & catalogue, String choice, ifstream is) {
	...
	case "ville": {
		//ask
		char constraint;
		cout << "Souhaitez-vous imposer une ville de dÈpart (D), d'arrivÈe (A) ou les deux (B) ?"<< endl;
		cout << "Veuillez saisir la lettre correspondant ‡ vtre choix" << endl;
		cin >> constraint;

		switch (constraint) {
		case 'D': {
			cout << "Quelle doit Ítre la ville de dÈpart des trajets ‡ sÈlectionner ?"<<endl;
		}
		case 'A': {
			cout << "Quelle doit Ítre la ville d'arrivÈe des trajets ‡ sÈlectionner ?"<<endl;
